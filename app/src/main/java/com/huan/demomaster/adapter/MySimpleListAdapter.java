...
public class MySimpleListAdapter extends BaseAdapter {
	...
	public MySimpleListAdapter(Context mContext, List<News> mList) {
		this.mContext = mContext;
		this.mInflater = LayoutInflater.from(mContext);
		this.mList = (ArrayList<News>) mList;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			viewHolder.tv_isOriginal = (TextView) convertView
					.findViewById(com.huan.demomaster.R.id.tv_isOriginal);
		...
		News mLog = mList.get(position);
		MyApp.imageLoader.displayImage(mLog
				.getAuthor().getHeadimageurl(), viewHolder.iv_author_head);
		viewHolder.tv_isOriginal.setText(mLog.isOriginal()?"原创":"转载");
		viewHolder.tv_isOriginal.setBackground(mContext.getResources().getDrawable(mLog.isOriginal()?R.drawable.tv_bg_stroke_red:R.drawable.tv_bg_stroke_green));
	...
	class ViewHolder {
		TextView tv_isOriginal,tv_title,tv_author,tv_time;
